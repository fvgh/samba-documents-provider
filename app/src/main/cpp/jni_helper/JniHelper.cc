...
static jobject create_directory_entry (JNIEnv* env,
  ...
  static const jmethodID dirEntryConstructor = env->GetMethodID (
      dirEntryClass, "<init>", "(ILjava/lang/String;Ljava/lang/String;)V");
  ...
  entry = env->NewObject (dirEntryClass, dirEntryConstructor, ent.smbc_type,
                          comment, name);

  env->DeleteLocalRef (name);
  bail: env->DeleteLocalRef (comment);
...
static void throw_new_file_not_found_exception (JNIEnv *env, const char *fmt,
  ...
  static const jclass fileNotFoundExceptionClass = classCache_.get (
...
static void throw_new_errno_exception (JNIEnv *env, const char *functionName_,
  ...
  static const jmethodID errnoExceptionConstructor = env->GetMethodID (
      errnoExceptionClass, "<init>", "(Ljava/lang/String;I)V");
  ...
  jthrowable errnoException = reinterpret_cast<jthrowable> (env->NewObject (
      errnoExceptionClass, errnoExceptionConstructor, functionName, err));
...
static void throw_new_auth_failed_exception (JNIEnv* env) {
  ...
  jthrowable authFailedException =
      reinterpret_cast<jthrowable> (env->NewObject (
          authFailedExceptionClass, authFailedExceptionConstructor));
  ...
  if (fd < 0)
    {
      ...
      switch (err)
        ...
        case ENODEV:
        case ENOENT:
          throw_new_file_not_found_exception (env,
                                              "Directory at %s can't be found.",
                                              uri);
          break;
        case EACCES:
        case EPERM:
...
static jobject create_structstat (JNIEnv *env, const struct stat &st) {
  ...
  static const jmethodID structStatConstructor = env->GetMethodID (
      structStatClass, "<init>", "(JJIJIIJJJJJJJ)V");

  return env->NewObject (structStatClass, structStatConstructor,
...
jobject Java_com_google_android_sambadocumentsprovider_nativefacade_NativeSambaFacade_stat (
  ...
  bail: env->ReleaseStringUTFChars (uri_, uri);
  ...
  if (fd < 0)
    {
      ...
      switch (err)
        ...
        case ENODEV:
        case ENOENT:
          throw_new_file_not_found_exception (env, "File at %s can't be found.",
                                              uri);
          break;
        case EACCES:
...
jlong Java_com_google_android_sambadocumentsprovider_nativefacade_SambaFile_read (
  ...
  ssize_t size = client->ReadFile (fd, buffer,
                                   static_cast<const size_t> (maxlen));
  if (size < 0)
    {
...
jlong Java_com_google_android_sambadocumentsprovider_nativefacade_SambaFile_write (
  ...
  ssize_t size = client->WriteFile (fd, buffer,
  ...
  if (password == NULL)
...

    {
      ...
      SambaClient::CredentialTuple tuple =
        { std::string (workgroup), std::string (username), std::string (
            password) };
      cache->put (uri, tuple);
    }

  env->ReleaseStringUTFChars (password_, password);
  bail_password: env->ReleaseStringUTFChars (username_, username);
  bail_username: env->ReleaseStringUTFChars (workgroup_, workgroup);
...
void Java_com_google_android_sambadocumentsprovider_SambaConfiguration_setEnv (
  ...
  bail: env->ReleaseStringUTFChars (var_, var);
}
